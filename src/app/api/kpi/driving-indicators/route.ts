...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Driving Indicators API] Fetching data for ${monthName} ${year}`);

    // Forward to Python backend
    const backendUrl = `${BACKEND_URL}/api/kpi/driving-indicators?month=${encodeURIComponent(monthName)}&year=${encodeURIComponent(year)}`;
    ...
    console.log(`[Driving Indicators API] Successfully fetched data`);

    return NextResponse.json(data);
  } catch (error) {
    console.error('[Driving Indicators API] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch driving indicators data' },
      { status: 500 }
    );
  }
}
