...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Market Share API] Fetching data for ${monthName} ${year}`);

    // Forward to Python backend
    const backendUrl = `${BACKEND_URL}/api/kpi/market-share?month=${encodeURIComponent(monthName)}&year=${encodeURIComponent(year)}`;
    ...
    console.log(`[Market Share API] Successfully fetched data`);
