...
// Lambda API Gateway URL (can be overridden via environment variable)
const BACKEND_URL = process.env.LAMBDA_KPI_API_URL || 'https://g3hjzqs2wf.execute-api.eu-west-1.amazonaws.com/dev';

// Increase timeout for this route to 60 seconds (Net Revenue data is large)
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Net Revenue API] Fetching data for ${monthName} ${year}`);

    // Forward to Python backend
    const backendUrl = `${BACKEND_URL}/api/kpi/net-revenue?month=${encodeURIComponent(monthName)}&year=${encodeURIComponent(year)}`;
    ...
    console.log(`[Net Revenue API] Successfully fetched data`);

    return NextResponse.json(data);
  } catch (error) {
    console.error('[Net Revenue API] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch net revenue data' },
      { status: 500 }
    );
  }
}
