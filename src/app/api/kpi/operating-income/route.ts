...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Operating Income API] Fetching data for ${monthName} ${year}`);

    // Forward to Python backend
    const backendUrl = `${BACKEND_URL}/api/kpi/operating-income?month=${encodeURIComponent(monthName)}&year=${encodeURIComponent(year)}`;
    ...
    console.log(`[Operating Income API] Successfully fetched data`);

    return NextResponse.json(data);
  } catch (error) {
    console.error('[Operating Income API] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch operating income data' },
      { status: 500 }
    );
  }
}
