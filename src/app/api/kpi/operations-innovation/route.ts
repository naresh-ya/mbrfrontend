...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[Operations & Innovation API] Fetching data for ${monthName} ${year}`);

    // Forward to Python backend
    const backendUrl = `${BACKEND_URL}/api/kpi/operations-innovation?month=${encodeURIComponent(monthName)}&year=${encodeURIComponent(year)}`;
    ...
    console.log(`[Operations & Innovation API] Successfully fetched data`);

    return NextResponse.json(data);
  } catch (error) {
    console.error('[Operations & Innovation API] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch operations & innovation data' },
      { status: 500 }
    );
  }
}
