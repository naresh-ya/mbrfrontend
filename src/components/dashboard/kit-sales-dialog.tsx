  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div
          ...
          "
        >
          {/* ✅ KPI Tiles - Special layout for Secondary Volume, Operating Income, and Net Revenue */}
          {data.id === "secondary-volume-sales" && (data as any).trendVsLast3Months ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">

              {/* Sub-Card 1: Trend vs Last 3 Months */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {(data as any).trendVsLast3Months.label}
                </span>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-lg font-semibold">
                    {(data as any).trendVsLast3Months.value}
                  </span>
                  {(data as any).trendVsLast3Months.trend === "increase" ? (
                    <ArrowUp className="h-3 w-3 text-green-600" />
                  ) : (
                    <ArrowDown className="h-3 w-3 text-red-500" />
                  )}
              ...
              {/* Sub-Card 2: Achievement vs Rolling Forecast */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {data.achievementVsRF.label}
                </span>
              ...
              {/* Sub-Card 3: Segment Mix */}
              <div className="p-2 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">Segment Mix</span>

                <div className="mt-2 space-y-1">
                  {(data as any).segmentMix?.items?.map((item: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between py-1"
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-medium text-gray-800 w-[60px]">
                          {item.segment}
                        </span>
                        <span className="text-[11px] font-semibold text-gray-900">
                          {item.value}
          ...
          ) : data.id === "operating-income" && (data as any).oiPercentOfNOR ? (
            /* Special layout for Operating Income */
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">

              {/* Sub-Card 1: OI % of NOR vs 3M Avg */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {(data as any).oiPercentOfNOR.label}
                </span>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-lg font-semibold">
                    {(data as any).oiPercentOfNOR.value}
                  </span>
                  {(data as any).oiPercentOfNOR.trend === "increase" ? (
                    <ArrowUp className="h-3 w-3 text-green-600" />
                  ) : (
                    <ArrowDown className="h-3 w-3 text-red-500" />
                  )}
                ...
                <p className="text-[11px] text-gray-600 mt-2 leading-tight">
                  {(data as any).oiPercentOfNOR.insight}
                </p>
              </div>

              {/* Sub-Card 2: Variance vs RF % */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {data.achievementVsRF.label}
                </span>
              ...
              {/* Sub-Card 3: Drivers (Monthly) with Component Percentages */}
              <div className="p-2 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">Drivers (Monthly)</span>

                <div className="mt-2 space-y-1">
                  {(data as any).drivers?.items?.map((driver: any, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between py-1"
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-medium text-gray-800 w-[50px]">
                          {driver.name}
                        </span>
                        <span className="text-[11px] font-semibold text-gray-900">
                          {driver.value}
                      ...
                      <span className={`text-[10px] font-medium ${
                        driver.impact === "positive" ? "text-green-600" : "text-red-600"
                      }`}>
                        {driver.percentage}
                      </span>
                    </div>
          ...
          ) : data.id === "net-revenue" && (data as any).last3MonthAvg ? (
            /* Special layout for Net Revenue - 4 sub-cards in 2x2 grid */
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">

              {/* Sub-Card 1: Last 3 Month Avg % */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {(data as any).last3MonthAvg.label}
                </span>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-lg font-semibold">
                    {(data as any).last3MonthAvg.value}
                  </span>
                  {(data as any).last3MonthAvg.trend === "increase" ? (
                    <ArrowUp className="h-3 w-3 text-green-600" />
                  ) : (
                    <ArrowDown className="h-3 w-3 text-red-500" />
                  )}
                ...
                <p className="text-[11px] text-gray-600 mt-2 leading-tight">
                  {(data as any).last3MonthAvg.insight}
                </p>
              </div>

              {/* Sub-Card 2: Achievement vs OB % */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">
                  {data.achievementVsOB.label}
                </span>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-lg font-semibold">
                    {data.achievementVsOB.value}
                  </span>
                  <Badge
                    className={`
                      text-[10px] px-2 py-[2px] rounded-full
                      ${
                        data.achievementVsOB.status === "On Track"
                          ? "bg-green-100 text-green-700 border border-green-300"
                          : data.achievementVsOB.status === "At Risk"
                          ? "bg-yellow-100 text-yellow-700 border border-yellow-300"
                          : "bg-red-100 text-red-700 border border-red-300"
                      }
                    `}
                  >
                    {data.achievementVsOB.status}
                  </Badge>
                </div>
              </div>
              ...
              {/* Sub-Card 4: Unit Economics */}
              <div className="p-3 rounded-lg border border-gray-200 bg-gray-50">
                <span className="text-[11px] text-gray-500">Unit Economics</span>

                <div className="mt-2 space-y-2">
                  <div>
                    <div className="text-[10px] text-gray-500">
                      {(data as any).unitEconomics.per000sBeforeSA.label}
                    </div>
                    <div className="text-[14px] font-semibold text-gray-900">
                      {(data as any).unitEconomics.per000sBeforeSA.value}
                    </div>
                  </div>

                  <div>
                    <div className="text-[10px] text-gray-500">
                      {(data as any).unitEconomics.per000sAfterSA.label}
                    </div>
                    <div className="text-[14px] font-semibold text-gray-900">
                      {(data as any).unitEconomics.per000sAfterSA.value}
                    </div>
                  </div>
                </div>
              </div>
   ...
   <p className="text-[11px] text-gray-600 mt-2 leading-tight">
        {data.achievementVsRF.text}
          ...
          {/* ✅ Insights Section - Only for Secondary Volume (Option B: bottom placement) */}
          {data.id === "secondary-volume-sales" && (data as any).insights && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
              <h4 className="flex items-center text-[12px] font-semibold text-gray-700">
                <Info className="mr-2 h-4 w-4 text-blue-600" />
                Key Insights
              ...
              <div className="mt-3 space-y-2">
                {/* Advance Indicator Insight */}
                {(data as any).insights.advanceIndicatorInsight && (
                ...
                {/* Segment Mix Insight */}
                {(data as any).insights.segmentMixInsight && (
                  <div className="text-[12px] text-gray-600 leading-relaxed">
                    <span className="font-medium text-gray-700">• </span>
                    {(data as any).insights.segmentMixInsight}
                  </div>
                ...
                {/* Executive Summary Bullets (first 3) */}
                {(data as any).insights.executiveSummary && (data as any).insights.executiveSummary.length > 0 && (
                  <div className="mt-2 space-y-2">
                    {(data as any).insights.executiveSummary.slice(0, 3).map((bullet: string, idx: number) => (
                      <div key={idx} className="text-[12px] text-gray-600 leading-relaxed">
                        <span className="font-medium text-gray-700">• </span>
                        {bullet}
                      </div>
                    ))}
