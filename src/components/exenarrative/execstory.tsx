...
function RevenueTrendChart() {
  return (
    <ResponsiveContainer width="100%" height={72}>
      <AreaChart data={REVENUE_DATA} margin={{ top: 4, right: 8, left: -28, bottom: 0 }}>
        <defs>
      ...
      <tbody>
        {STATE_DATA.map((r, i) => (
          <tr key={i} className={r.status === "warn" ? styles.rowWarn : ""}>
            ...
            <td>{r.share}</td>
            <td className={r.status === "warn" ? styles.cellRed : styles.cellGreen}>{r.seq}</td>
            <td>
              <span className={r.status === "warn" ? styles.tagWarn : r.status === "good" ? styles.tagGood : styles.tagNeutral}>
                {r.status === "warn" ? "\u26a0 Review" : r.status === "good" ? "\u2197 Accelerate" : "\u2192 Monitor"}
              </span>
            </td>
          </tr>
        ))}
  ...
  return (
    <div className={styles.launchTracker}>
      <div className={styles.launchHeader}>
        <span className={styles.launchTitle}>Compact Edge — Day 60 Scorecard</span>
        <span className={styles.launchStatus}>⚠ Below Target</span>
      </div>
      {LAUNCH_METRICS.map((m) => {
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.panel}>
            ...
            <div className={`${styles.callout} ${styles.calloutWarn}`}>
              <div className={styles.calloutTitle}>Why it matters</div>
              <div className={styles.calloutText}>
                If the trade shifts from stocking to liquidation, secondary volumes can dip sharply despite stable consumer demand — distorting P&amp;L signals.
              </div>
            </div>
